...
import { Calendar as CalendarIcon, MapPin, Upload, Plus, X ,ChevronDown } from 'lucide-react';
...
partnerDescription: '',
partnerLocation: '',
eventType: '', // Changed from array to string
eventSubType: '', // New state for event sub-type
...
eventType: true,
eventSubType: true, // New state for event sub-type section
eventDetails: true,
tags: true,
host: true,
partner: true,
...
useEffect(() => {
  const fetchMeta = async () => {
    try {
      const response = await fetch('http://44.203.188.5:3000/events/get-event-meta', {
        headers: {
          'accept': '*/*',
...
<form onSubmit={handleSubmit} className="space-y-8">
      ...
      <Card>
        ...
        {/* FIX: Remove raw HTML dropdown and use a Select component for event type selection */}
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            <input type="checkbox" checked={enabledSections.eventSubType ?? true} onChange={() => handleSectionToggle('eventSubType')} />
            <CardTitle>Event Subtype</CardTitle>
          </div>
          <CardDescription>Select a subtype for the event (optional)</CardDescription>
        </CardHeader>
        {enabledSections.eventSubType !== false && (
          <CardContent>
                    ...
                    <div key={idx} className="p-4 border rounded-lg shadow-sm bg-gray-50">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Select
                            value={req.type}
                            onValueChange={value => handleEntryTypeChange(value as 'string' | 'array')}
                          >
